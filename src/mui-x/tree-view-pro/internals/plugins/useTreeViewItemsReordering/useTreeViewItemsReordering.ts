...
> = ({ params, store }) => {
  ...
  const getDroppingTargetValidActions = React.useCallback(
    (itemId: string) => {
      const currentReorder = itemsReorderingSelectors.currentReorder(
        store.state,
      );
      if (!currentReorder) {
      ...
      const targetItemIndex = itemsSelectors.itemIndex(
        store.state,
        targetItemMeta.id,
      );
      ...
      const draggedItemIndex = itemsSelectors.itemIndex(
        store.state,
        draggedItemMeta.id,
      );
      ...
      Object.keys(positionsAfterAction).forEach((action) => {
        const positionAfterAction =
          positionsAfterAction[action as TreeViewItemsReorderingAction];
        if (
          positionAfterAction != null &&
          checkIfPositionIsValid(positionAfterAction)
        ) {
          validActions[action as TreeViewItemsReorderingAction] =
            positionAfterAction;
        }
      });
  ...
  const startDraggingItem = React.useCallback(
    (itemId: string) => {
      const isItemBeingEdited = labelSelectors.isItemBeingEdited(
        store.state,
        itemId,
      );
  ...
  >(
    ({
      ...
      if (
        prevItemReorder == null ||
        isAncestor(store, itemId, prevItemReorder.draggedItemId)
      ) {
        return;
      }
