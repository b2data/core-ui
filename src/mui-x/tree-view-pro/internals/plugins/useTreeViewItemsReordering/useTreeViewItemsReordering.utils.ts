...
export const chooseActionToApply = ({
  ...
  if (
    validActions["move-to-parent"] &&
    cursorX < itemChildrenIndentationPx * targetDepth
  ) {
  ...
  // - the inner half makes it a child
    ...
    if (validActions["reorder-above"] && cursorY < (1 / 4) * targetHeight) {
      ...
      validActions["reorder-below"] &&
      cursorY > (3 / 4) * targetHeight
    ) {
...
export const moveItemInTree = <R extends {}>({
  ...
  if (oldParentId === newParentId) {
    ...
    itemOrderedChildrenIds[
      itemToMoveMeta.parentId ?? TREE_VIEW_ROOT_PARENT_ID
    ] = updatedChildren;
  } else {
    ...
    const updatedNewParentChildren = [
      ...(itemOrderedChildrenIds[newParentId] ?? []),
    ];
  ...
  itemChildrenIndexes[oldParentId] = buildSiblingIndexes(
    itemOrderedChildrenIds[oldParentId],
  );
  if (newParentId !== oldParentId) {
    itemChildrenIndexes[newParentId] = buildSiblingIndexes(
      itemOrderedChildrenIds[newParentId],
    );
  }
  ...
  const itemToMoveDepth =
    newPosition.parentId == null ? 0 : itemMetaLookup[newParentId].depth! + 1;
  ...
  const updateItemDepth = (itemId: string, depth: number) => {
    ...
    itemOrderedChildrenIds[itemId]?.forEach((childId) =>
      updateItemDepth(childId, depth + 1),
    );
  };
