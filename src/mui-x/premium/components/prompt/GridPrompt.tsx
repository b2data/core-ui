...
const PromptItem = styled("li", {
  ...
  &:hover
    .${gridClasses.promptAction},
    &
    .${gridClasses.promptAction}:focus-visible {
    opacity: 1;
  }
  @media (prefers-reduced-motion: no-preference) {
    animation: ${fadeInUp} ${vars.transitions.duration.long}
...
const PromptIcon = styled("svg", {
  ...
  color: ${({ ownerState }) =>
    ownerState.variant === "error"
      ? vars.colors.foreground.error
      : vars.colors.foreground.muted};
  @media (prefers-reduced-motion: no-preference) {
    animation: ${fadeIn} ${vars.transitions.duration.short}
      ${vars.transitions.easing.easeInOut};
  }
`;
  ...
  const getGroupingChanges = React.useCallback(
    (grouping: PromptResponse["grouping"]) => {
      return grouping.map((group) => ({
        ...
        description: apiRef.current.getLocaleText(
  ...
  const getAggregationChanges = React.useCallback(
    (aggregation: PromptResponse["aggregation"]) => {
      return Object.keys(aggregation).map((column) => ({
        ...
        )(getColumnName(column), aggregation[column]),
  ...
  const getSortingChanges = React.useCallback(
    (sorting: PromptResponse["sorting"]) => {
      return sorting.map((sort) => ({
        ...
        )(getColumnName(sort.column), sort.direction),
        icon:
          sort.direction === "asc"
  ...
  const getPivotingChanges = React.useCallback(
    (pivoting: PromptResponse["pivoting"]) => {
        ...
        changes.push({
          ...
          description: pivoting.columns
            .map((column) =>
              ...
              )(getColumnName(column.column), column.direction),
            )
            .join(`, `),
        });
      }
      ...
      if (pivoting.rows.length) {
        changes.push({
          label: apiRef.current.getLocaleText("promptChangePivotRowsLabel")(
            pivoting.rows.length,
          ),
          icon: rootProps.slots.densityStandardIcon,
          description: pivoting.rows
