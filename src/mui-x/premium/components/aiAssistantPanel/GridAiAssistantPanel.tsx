...
function GridAiAssistantPanel() {
  ...
  const activeConversation = useGridSelector(
    apiRef,
    gridAiAssistantActiveConversationSelector,
  );
  const conversations = useGridSelector(
    apiRef,
    gridAiAssistantConversationsSelector,
  );
  const conversationTitle =
    activeConversation?.title ||
    apiRef.current.getLocaleText("aiAssistantPanelNewConversation");

  const createConversation = React.useCallback(() => {
    const newConversation = conversations.findIndex(
      (conversation) => !conversation.prompts.length,
    );
    if (newConversation !== -1) {
      ...
      apiRef.current.aiAssistant.setActiveConversationIndex(
  ...
  return (
    <AiAssistantPanelRoot className={classes.root} ownerState={rootProps}>
        ...
        >
          <span>
            <rootProps.slots.baseIconButton
              ...
              disabled={
                !conversations.length || !activeConversation?.prompts.length
              }
      ...
      <AiAssistantPanelFooter className={classes.footer} ownerState={rootProps}>
        ...
        {rootProps.aiAssistantSuggestions &&
          rootProps.aiAssistantSuggestions.length > 0 && (
