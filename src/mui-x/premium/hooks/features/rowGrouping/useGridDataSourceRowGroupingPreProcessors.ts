    ...
    (params) => {
      ...
      const getRowTreeBuilderNode = (rowId: GridRowId) => {
        ...
        return {
          ...
          serverChildrenCount:
            getChildrenCount(params.dataRowIdToModelLookup[rowId]) ?? 0,
        };
      };
      ...
      return updateRowTree({
        nodes: {
          inserted: (
            params.updates as GridRowsPartialUpdates
          ).actions.insert.map(getRowTreeBuilderNode),
          modified: (
            params.updates as GridRowsPartialUpdates
          ).actions.modify.map(getRowTreeBuilderNode),
