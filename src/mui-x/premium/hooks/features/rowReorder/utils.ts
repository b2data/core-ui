...
import type {
  ReorderValidationContext as Ctx,
...
export const conditions = {
  ...
  targetDepthIsSourceMinusOne: (ctx: Ctx) =>
  ...
  targetGroupExpanded: (ctx: Ctx) =>
    (ctx.targetNode.type === "group" &&
      (ctx.targetNode as GridGroupNode).childrenExpanded) ??
    false,
  ...
  prevDepthEqualsSource: (ctx: Ctx) =>
    ctx.prevNode?.depth === ctx.sourceNode.depth,
  ...
  targetFirstChildIsGroupWithSourceDepth: (ctx: Ctx) => {
    ...
    const firstChild = targetGroup.children?.[0]
      ? ctx.rowTree[targetGroup.children[0]]
      : null;
    return (
      firstChild?.type === "group" && firstChild.depth === ctx.sourceNode.depth
    );
  },
...
export function removeEmptyAncestors(
  ...
  while (currentGroupId && currentGroupId !== GRID_ROOT_GROUP_ID) {
    ...
    const remainingChildren = group.children.filter(
      (childId) => !removedGroups.has(childId),
    );
...
export class BatchRowUpdater {
  ...
  constructor(
    private processRowUpdate:
      | DataGridPremiumProcessedProps["processRowUpdate"]
      | undefined,
