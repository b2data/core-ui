...
export class SameParentSwapOperation extends BaseReorderOperation {
  ...
  detectOperation(ctx: ReorderExecutionContext): ReorderOperation | null {
    ...
    if (
      targetIndex === sortedFilteredRowIds.length &&
      sortedFilteredRowIds.length > 0
    ) {
      targetIndex -= 1;
    }

    let targetNode = gridRowNodeSelector(
      apiRef,
      sortedFilteredRowIds[targetIndex],
    );
  ...
  executeOperation(
    ...
    apiRef.current.setState((state: any) => {
      const group = gridRowTreeSelector(apiRef)[
        sourceNode.parent!
      ] as GridGroupNode;
      ...
      currentChildren.splice(
        actualTargetIndex,
        0,
        currentChildren.splice(oldIndex, 1)[0],
      );
...
export class CrossParentLeafOperation extends BaseReorderOperation {
  ...
  detectOperation(ctx: ReorderExecutionContext): ReorderOperation | null {
    ...
    let targetNode = gridRowNodeSelector(
      apiRef,
      sortedFilteredRowIds[placeholderIndex],
    );
    let isLastChild = false;
  ...
  async executeOperation(
    ...
    const { apiRef, sourceRowId, processRowUpdate, onProcessRowUpdateError } =
      ctx;
    ...
    const commitStateUpdate = (finalSourceRow: GridValidRowModel) => {
      apiRef.current.setState((state: any) => {
        const updatedSourceChildren = sourceChildren.filter(
          (rowId) => rowId !== sourceRowId,
        );
        ...
        return {
          ...
          rows: {
            ...
            totalTopLevelRowCount:
              state.rows.totalTopLevelRowCount - rootLevelRemovals,
            tree: updatedTree,
          },
        };
      });
    ...
    if (processRowUpdate && !isDeepEqual(originalSourceRow, updatedSourceRow)) {
      ...
      try {
        const processedRow = await processRowUpdate(
          updatedSourceRow,
          originalSourceRow,
          params,
        );
...
export class CrossParentGroupOperation extends BaseReorderOperation {
  ...
  async executeOperation(
    ...
    const allLeafIds = collectAllLeafDescendants(
    ...
    const updater = new BatchRowUpdater(
      processRowUpdate,
    ...
    for (const leafId of allLeafIds) {
      ...
      for (let depth = 0; depth < targetParentPath.length; depth += 1) {
        ...
        if (pathItem.field) {
          const groupingRule = groupingRules.find(
            (rule) => rule.field === pathItem.field,
          );
          if (groupingRule) {
            const colDef = columnsLookup[groupingRule.field];
            if (groupingRule.groupingValueSetter && colDef) {
    ...
    try {
      ...
      if (successful.length > 0) {
        apiRef.current.setState((state: any) => {
          ...
          if (failed.length === 0) {
            const sourceParentNode = updatedTree[
              sourceNode.parent!
            ] as GridGroupNode;

            if (!sourceParentNode) {
              const targetParentNode = updatedTree[
                targetNode.parent!
              ] as GridGroupNode;
              const targetIndex = targetParentNode.children.indexOf(
                targetNode.id,
              );
              ...
              const updatedSourceParentChildren =
                sourceParentNode.children.filter((id) => id !== sourceNode.id);

              if (updatedSourceParentChildren.length === 0) {
                const removedGroups = new Set<GridRowId>();
                removedGroups.add(sourceNode.parent!);

                const parentOfSourceParent = (
                  updatedTree[sourceNode.parent!] as GridGroupNode
                ).parent;
              ...
              const existingGroup =
                sourceGroupNode.groupingKey !== null &&
                sourceGroupNode.groupingField !== null
                  ? findExistingGroupWithSameKey(
                      targetParentNode,
