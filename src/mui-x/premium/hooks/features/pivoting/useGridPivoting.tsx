...
> = (state, props, apiRef) => {
  ...
  const open =
    props.pivotPanelOpen ?? props.initialState?.pivoting?.panelOpen ?? false;
  ...
  return {
    ...
    pivoting: {
      active:
        props.pivotActive ?? props.initialState?.pivoting?.enabled ?? false,
      model:
        props.pivotModel ??
        props.initialState?.pivoting?.model ??
        emptyPivotModel,
      initialColumns,
    },
    sidebar: {
...
export const useGridPivoting = (
  ...
  const computePivotingState = React.useCallback(
    ({
      active,
      model: pivotModel,
    }: Pick<GridPivotingState, "active" | "model">) => {
  ...
  const updatePivotModel = React.useCallback<
    GridPivotingPrivateApi["updatePivotModel"]
  >(
    ({
      field,
      targetSection,
      ...
      apiRef.current.setPivotModel((prev) => {
        ...
        const hidden =
          originSection === null
            ? false
            : (prev[originSection].find((item) => item.field === field)
                ?.hidden ?? false);
        ...
        if (targetSection) {
          ...
          if (targetField) {
            const fromIndex = newSectionArray.findIndex(
              (item) => item.field === field,
            );
            if (fromIndex > -1) {
              newSectionArray.splice(fromIndex, 1);
            }
            toIndex = newSectionArray.findIndex(
        ...
        if (!isSameSection && originSection) {
          (newModel[originSection] as (typeof prev)[typeof originSection]) =
            prev[originSection].filter((f) => f.field !== field);
        }
        return newModel;
      });
    },
  ...
  const setPivotActive = React.useCallback<GridPivotingApi["setPivotActive"]>(
    (callback) => {
      ...
      apiRef.current.setState((state) => {
        const newPivotMode =
          typeof callback === "function"
            ? callback(state.pivoting?.active)
            : callback;
  ...
  const setPivotPanelOpen = React.useCallback<
    GridPivotingApi["setPivotPanelOpen"]
  >(
    (callback) => {
      if (!isPivotingAvailable) {
        return;
      ...
      const newPanelOpen =
        typeof callback === "function" ? callback(panelOpen) : callback;
  ...
  const addColumnMenuButton = React.useCallback<
  ...
  const updateNonPivotColumns = React.useCallback<
    GridPivotingPrivateApi["updateNonPivotColumns"]
  >(
  ...
  const updateNonPivotRows = React.useCallback<
    GridPivotingPrivateApi["updateNonPivotRows"]
  >(
  ...
  useGridApiMethod(
    apiRef,
    { setPivotModel, setPivotActive, setPivotPanelOpen },
    "public",
  );
  useGridApiMethod(
    apiRef,
...
export const useGridPivotingExportState = (
  apiRef: RefObject<GridPrivateApiPremium>,
) => {
  const stateExportPreProcessing: GridPipeProcessor<"exportState"> =
    React.useCallback(
