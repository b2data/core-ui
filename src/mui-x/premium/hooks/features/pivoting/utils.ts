      ...
      return [
        {
          ...
          valueGetter: (_value, row) => new Date(row[field]).getFullYear(),
        },
...
export const getInitialColumns = (
  ...
  for (let i = 0; i < originalColumns.length; i += 1) {
    ...
    const column = {
      ...getDefaultColTypeDef(originalColumn.type),
      ...originalColumn,
    };
    const field = column.field;
...
export const getPivotedData = ({
      ...
      for (let j = 0; j < visibleColumns.length; j += 1) {
        ...
        if (!columnGroupingModelLookup.has(groupId)) {
          ...
          if (depth === 0) {
            ...
            const parentGroupId = columnGroupPath
              .slice(0, -1)
              .join(columnGroupIdSeparator);
          ...
          visibleValues.forEach((pivotValue) => {
            ...
            const overrides =
              typeof pivotingColDef === "function"
                ? pivotingColDef(
                    valueField,
                    columnGroup.groupId.split(columnGroupIdSeparator),
                  )
                : pivotingColDef;
  ...
  const groupingColDefOverrides = (
    params: GridGroupingColDefOverrideParams,
  ) => ({
    ...(typeof groupingColDef === "function"
