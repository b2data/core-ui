...
export const aiAssistantStateInitializer: GridStateInitializer<
  Pick<
    ...
    "initialState" | "aiAssistantConversations" | "aiAssistant"
  >
> = (state, props) => {
  ...
  return {
    ...
    aiAssistant: {
      activeConversationIndex: 0,
      conversations:
        props.aiAssistantConversations ??
        props.initialState?.aiAssistant?.conversations ??
        [],
    } as GridAiAssistantState,
  };
};
...
export const useGridAiAssistant = (
  ...
  const getPromptContext = React.useCallback(
    (allowDataSampling = false) => {
      ...
      const columnsContext = columns.map((column) => ({
        ...
        allowedOperators:
          column.filterOperators?.map((operator) => operator.value) ?? [],
      }));
    ...
    (result: PromptResponse) => {
        ...
        apiRef.current.setFilterModel({
          items: result.filters.map((filter, index) => {
            ...
            if (column.type === "singleSelect") {
              const options =
                getValueOptions(column as GridSingleSelectColDef) ?? [];
              const found = options.find(
                (option) =>
                  typeof option === "object" && option.label === filter.value,
              );
              if (found) {
          ...
          logicOperator:
            (result.filterOperator as GridLogicOperator) ??
      ...
      if (!disableRowGrouping && !appliedPivoting) {
        apiRef.current.setRowGroupingModel(
          result.grouping.map((g) => g.column),
        );
      }
      ...
      interestColumns
        .reverse()
        .forEach((c) => apiRef.current.setColumnIndex(c, targetIndex));
    },
    [
      apiRef,
      disableColumnFilter,
  ...
  const setActiveConversationId = React.useCallback(
    (id: string) => {
      ...
      const activeConversationIndex =
        gridAiAssistantActiveConversationIndexSelector(apiRef);
  ...
  const processPrompt = React.useCallback(
    async (value: string) => {
      ...
      const activeConversation =
        gridAiAssistantActiveConversationSelector(apiRef);
      const date = Date.now();
  ...
  const setConversations = React.useCallback<
    GridAiAssistantApi["aiAssistant"]["setConversations"]
  >(
    (callback) => {
      if (!isAiAssistantAvailable) {
        return;
      ...
      apiRef.current.setState((state) => ({
        ...
        aiAssistant: {
          ...
          conversations:
            typeof callback === "function"
              ? callback(state.aiAssistant?.conversations)
              : callback,
        },
      }));
    },
  ...
  React.useEffect(() => {
    ...
    apiRef,
    props.aiAssistantActiveConversationIndex,
