  ...
  for (let i = 0; i < valueOptions.length; i += 1) {
    ...
    if (valueFormatter) {
      if (typeof option === "object") {
        ...
        value = String(
          colDef.valueFormatter!(option as never, row, colDef, {
            current: api,
          }),
        );
      }
  ...
  columns.forEach((column, colIndex) => {
    ...
    switch (column.type) {
      case "singleSelect": {
          ...
          getFormattedValueOptions(
            ...
            (value, index) => {
              const formatted = value
                .toString()
                .replace(commaRegex, commaReplacement);
              formulae += formatted;
        ...
        const formattedValue = apiRef.current.getRowFormattedValue(
          row,
          castColumn,
        );
