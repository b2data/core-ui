...
function transformTheme(t: Theme): GridCSSVariablesInterface {
  ...
  const fontBody =
    (t.vars as any)?.font?.body2 ?? formatFont(t.typography.body1);
  const fontSmall =
    (t.vars as any)?.font?.caption ?? formatFont(t.typography.caption);
  const fontLarge =
    (t.vars as any)?.font?.body1 ?? formatFont(t.typography.body1);
  const k = vars.keys;

  return {
    [k.spacingUnit]: t.vars
      ? ((t.vars as any).spacing ?? t.spacing(1))
      : t.spacing(1),
    ...
    [k.colors.interactive.focusOpacity]: (t.vars || t).palette.action
      .focusOpacity,
    [k.colors.interactive.disabled]: removeOpacity(
      (t.vars || t).palette.action.disabled,
    ),
    [k.colors.interactive.disabledOpacity]: (t.vars || t).palette.action
      .disabledOpacity,
    [k.colors.interactive.selected]: selectedColor,
    [k.colors.interactive.selectedOpacity]: (t.vars || t).palette.action
      .selectedOpacity,
