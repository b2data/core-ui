...
export function useGridVirtualizer(
  ...
  const groupHeaderHeight = Math.floor(
    (rootProps.columnGroupHeaderHeight ?? rootProps.columnHeaderHeight) *
      density,
  );
  ...
  const rightPinnedWidth = pinnedColumns.right.reduce(
    (w, col) => w + col.computedWidth,
    0,
  );
  ...
  const virtualizer = useVirtualizer({
    ...
    colspan: {
      ...
      getColspan: (rowId, column) => {
        if (typeof column.colSpan === "function") {
          ...
          const value = apiRef.current.getRowValue(
            row,
    ...
    getRowHeight: React.useMemo(() => {
      ...
      return (rowEntry) =>
        getRowHeight({ ...rowEntry, densityFactor: density });
    }, [getRowHeight, density]),
    getEstimatedRowHeight: React.useMemo(
      () =>
        getEstimatedRowHeight
          ? (rowEntry) =>
              getEstimatedRowHeight({ ...rowEntry, densityFactor: density })
          : undefined,
        ...
        getRowSpacing
          ? (rowEntry) => {
              const indexRelativeToCurrentPage =
                currentPage.rowIdToIndexMap.get(rowEntry.id) ?? -1;
              ...
              return getRowSpacing({
                ...
                ...visibility,
                indexRelativeToCurrentPage:
                  apiRef.current.getRowIndexRelativeToVisibleRows(rowEntry.id),
              });
            }
          : undefined,
    ...
    onResize: useEventCallback((size) =>
    ...
    renderInfiniteLoadingTrigger: (id) =>
      (apiRef as any).current.getInfiniteLoadingTriggerElement?.({
        lastRowId: id,
      }),
  });
  ...
  useFirstRender(() => {
    ...
    apiRef.current.store.state.virtualization =
