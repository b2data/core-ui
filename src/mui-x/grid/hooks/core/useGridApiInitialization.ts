...
function createPrivateAPI<
  PrivateApi extends GridPrivateApiCommon,
  Api extends GridApiCommon,
>(publicApiRef: RefObject<Api>): PrivateApi {
  const existingPrivateApi = (publicApiRef.current as any)?.[
    SYMBOL_API_PRIVATE
  ];
  ...
  privateApi.register = (visibility, methods) => {
    Object.keys(methods).forEach((methodName) => {
      ...
      const currentPrivateMethod = privateApi[
        methodName as keyof typeof privateApi
      ] as any;
      ...
      if (visibility === "public") {
        ...
        const currentPublicMethod = publicApi[
          methodName as keyof typeof publicApi
        ] as any;
...
function createPublicAPI<
  PrivateApi extends GridPrivateApiCommon,
  Api extends GridApiCommon,
>(privateApiRef: RefObject<PrivateApi>): Api {
  const publicApi = {
    get state() {
...
>(
  ...
  useGridApiMethod(
    privateApiRef,
    { subscribeEvent, publishEvent } as any,
    "public",
  );
  ...
  React.useImperativeHandle(inputApiRef, () => publicApiRef.current, [
    publicApiRef,
  ]);
