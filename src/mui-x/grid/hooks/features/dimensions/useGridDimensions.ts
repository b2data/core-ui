...
export function useGridDimensions(
  ...
  const getViewportPageSize = React.useCallback(() => {
    ...
    if (props.getRowHeight) {
      ...
      const viewportPageSize =
        renderContext.lastRowIndex - renderContext.firstRowIndex;
  ...
  const updateDimensions = React.useCallback(() => {
    ...
    const topContainerHeight =
      headersTotalHeight + rowsMeta.pinnedTopRowsTotalHeight;
    ...
    if (props.autoHeight) {
      hasScrollY = false;
      hasScrollX =
        Math.round(columnsTotalWidth) >
        Math.round(rootDimensionsRef.current.width);
...
function getStaticDimensions(
  ...
  return {
    ...
    leftPinnedWidth: pinnedColumnns.left.reduce(
      (w, col) => w + col.computedWidth,
      0,
    ),
    rightPinnedWidth: pinnedColumnns.right.reduce(
      (w, col) => w + col.computedWidth,
      0,
    ),
  };
