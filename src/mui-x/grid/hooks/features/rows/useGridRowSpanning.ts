...
const computeRowSpanningState = (
  ...
  const spannedCells = resetState
    ? {}
    : { ...apiRef.current.state.rowSpanning.spannedCells };
  const hiddenCells = resetState
    ? {}
  ...
  colDefs.forEach((colDef) => {
    ...
    processedRange = {
      firstRowIndex: Math.min(
        processedRange.firstRowIndex,
        rangeToProcess.firstRowIndex,
      ),
      lastRowIndex: Math.max(
        processedRange.lastRowIndex,
...
export const rowSpanningStateInitializer: GridStateInitializer = (
  ...
  const { spannedCells, hiddenCells, hiddenCellOriginMap } =
  ...
  const updateRowSpanningState = React.useCallback(
    (renderContext: GridRenderContext, resetState: boolean = false) => {
      ...
      const hasNoSpannedCells =
        newSpannedCellsCount === 0 && currentSpannedCellsCount === 0;
