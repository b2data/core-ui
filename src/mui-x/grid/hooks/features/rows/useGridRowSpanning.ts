...
const computeRowSpanningState = (
  ...
  const previousState = resetState
    ? EMPTY_STATE
    : Rowspan.selectors.state(virtualizer.store.state);
  ...
  const processedRange = {
    ...
    lastRowIndex: Math.max(
      previousState.processedRange.lastRowIndex,
      rangeToProcess.lastRowIndex,
    ),
  };
  ...
  colDefs.forEach((colDef, columnIndex) => {
    for (
      ...
      while (
        ...
        getCellValue(visibleRows[relativeIndex].model, colDef, apiRef) ===
          cellValue
      ) {
 ...
 */
export const rowSpanningStateInitializer: GridStateInitializer = (
  state,
  props,
  apiRef,
) => {
  ...
  const colDefs = orderedFields.map(
    (field) => columnsLookup[field!],
  ) as GridColDef[];
...
export const useGridRowSpanning = (
  ...
  props: Pick<
    ...
    "rowSpanning" | "pagination" | "paginationMode"
  >,
): void => {
  ...
  const updateRowSpanningState = React.useCallback(
    (renderContext: GridRenderContext, resetState: boolean = false) => {
      ...
      const previousState = resetState
        ? EMPTY_STATE
        : Rowspan.selectors.state(store.state);
      ...
      const newSpannedCellsCount = Object.keys(
        newState.caches.spannedCells,
      ).length;
      const newHiddenCellsCount = Object.keys(
        newState.caches.hiddenCells,
      ).length;
      const previousSpannedCellsCount = Object.keys(
        previousState.caches.spannedCells,
      ).length;
      const previousHiddenCellsCount = Object.keys(
        previousState.caches.hiddenCells,
      ).length;
      ...
      const hasNoSpannedCells =
        newSpannedCellsCount === 0 && previousSpannedCellsCount === 0;
