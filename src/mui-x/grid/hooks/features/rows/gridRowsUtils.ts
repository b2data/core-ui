...
> & {
  ...
  const groupingParamsWithHydrateRows =
    apiRef.current.unstable_applyPipeProcessors("hydrateRows", {
      tree: unProcessedTree,
      treeDepths: unProcessedTreeDepths,
      dataRowIds: unProcessedDataRowIds,
      dataRowIdToModelLookup: cache.dataRowIdToModelLookup,
  ...
  return {
    ...
    totalRowCount: Math.max(
      rowCountProp,
      groupingParamsWithHydrateRows.dataRowIds.length,
    ),
...
export const updateCacheWithNewRows = ({
  ...
  uniqueUpdates.forEach((partialRow, id) => {
    ...
    if (partialRow._action === "delete") {
      ...
      // Then do nothing.
      if (
        actionAlreadyAppliedToRow === "remove" ||
        !dataRowIdToModelLookup[id]
      ) {
        return;
      }
  ...
  for (let i = 0; i < actionTypeWithActionsToRemove.length; i += 1) {
    ...
    if (Object.keys(idsToRemove).length > 0) {
      partialUpdates.actions[actionType] = partialUpdates.actions[
        actionType
  ...
  return {
    ...
    loadingPropBeforePartialUpdates:
      previousCache.loadingPropBeforePartialUpdates,
    rowCountPropBeforePartialUpdates:
      previousCache.rowCountPropBeforePartialUpdates,
  };
};

export const minimalContentHeight =
  "var(--DataGrid-overlayHeight, calc(var(--height) * 2))";
