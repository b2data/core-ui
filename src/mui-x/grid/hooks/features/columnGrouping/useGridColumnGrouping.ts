...
const createGroupLookup = (
  columnGroupingModel: GridColumnNode[],
  ...
  for (let i = 0; i < columnGroupingModel.length; i += 1) {
    ...
    if (
      subTreeLookup[groupId] !== undefined ||
      groupLookup[groupId] !== undefined
    ) {
      throw new Error(
    ...
    visibleColumnFields.length === 0
      ...
      : Math.max(
          ...visibleColumnFields.map(
            (field) => unwrappedGroupingModel[field]?.length ?? 0,
          ),
        );
...
export const useGridColumnGrouping = (
  ...
  const getColumnGroupPath = React.useCallback<
    GridColumnGroupingApi["getColumnGroupPath"]
  >(
    (field) => {
      const unwrappedGroupingModel =
  ...
  const getAllGroupDetails = React.useCallback<
    GridColumnGroupingApi["getAllGroupDetails"]
  >(() => {
  ...
  const handleColumnIndexChange = React.useCallback<
    GridEventListener<"columnIndexChange">
  >(() => {
    const unwrappedGroupingModel = unwrapGroupingColumnModel(
      props.columnGroupingModel ?? [],
    );
  ...
  const updateColumnGroupingState = React.useCallback(
    (columnGroupingModel: GridColumnGroupingModel | undefined) => {
      apiRef.current.caches.columnGrouping.lastColumnGroupingModel =
        columnGroupingModel;
