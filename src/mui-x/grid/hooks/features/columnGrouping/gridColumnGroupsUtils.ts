...
export const getColumnGroupsHeaderStructure = (
  ...
  const maxDepth = Math.max(
    ...orderedColumns.map((field) => getParents(field).length),
  );

  const haveSameParents = (field1: string, field2: string, depth: number) =>
    isDeepEqual(
      getParents(field1).slice(0, depth + 1),
      getParents(field2).slice(0, depth + 1),
    );
  ...
  for (let depth = 0; depth < maxDepth; depth += 1) {
    const depthStructure = orderedColumns.reduce((structure, newField) => {
      ...
      const prevField =
        lastGroup.columnFields[lastGroup.columnFields.length - 1];
