...
export const useGridDataSourceBase = <Api extends GridPrivateApiCommunity>(
  ...
  const cacheChunkManager = useLazyRef<CacheChunkManager, void>(() => {
    ...
    const sortedPageSizeOptions = props.pageSizeOptions
      ...
      .sort((a, b) => a - b);
    const cacheChunkSize = Math.min(
      paginationModel.pageSize,
      sortedPageSizeOptions[0],
    );
    ...
    async (parentId, params) => {
      ...
      if (!skipCache && responses.every((response) => response !== undefined)) {
        apiRef.current.applyStrategyProcessor("dataSourceRowsUpdate", {
          response: CacheChunkManager.mergeResponses(
            responses as GridGetRowsResponse[],
          ),
          fetchParams,
        });
        return;
      ...
      try {
        ...
        const cacheResponses = cacheChunkManager.splitResponse(
          fetchParams,
          getRowsResponse,
        );
  ...
  const editRow = React.useCallback<GridDataSourceApiBase["editRow"]>(
    async (params) => {
      ...
      try {
        ...
        if (
          finalRowUpdate &&
          !isDeepEqual(finalRowUpdate, params.previousRow)
        ) {
  ...
  return {
    ...
    events: {
      ...
      sortModelChange: runIf(defaultRowsUpdateStrategyActive, () =>
        debouncedFetchRows(),
      ),
      filterModelChange: runIf(defaultRowsUpdateStrategyActive, () =>
        debouncedFetchRows(),
      ),
      paginationModelChange: runIf(defaultRowsUpdateStrategyActive, () =>
        debouncedFetchRows(),
      ),
    },
  };
