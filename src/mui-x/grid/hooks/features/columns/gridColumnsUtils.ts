...
export function computeFlexColumnsWidth({
  ...
  const uniqueFlexColumns = new Set<GridColDef["field"]>(
    flexColumns.map((col) => col.field),
  );
...
export const hydrateColumnsWidth = (
  ...
  const initialFreeSpace = Math.max(
    availableWidth - widthAllocatedBeforeFlex,
    0,
  );
...
export const applyInitialState = (
  ...
  for (let i = 0; i < columnsWithUpdatedDimensions.length; i += 1) {
    ...
    Object.entries(dimensions[field]).forEach(([key, value]) => {
      newColDef[key as GridColumnDimensionProperties] =
...
export const createColumnsState = ({
  ...
  const columnsStateWithPreProcessing =
    apiRef.current.unstable_applyPipeProcessors("hydrateColumns", columnsState);
...
export function getTotalHeaderHeight(
  ...
  const columnHeadersHeight = Math.floor(
    props.columnHeaderHeight * densityFactor,
  );
  ...
  return (
    columnHeadersHeight +
    columnGroupHeadersHeight * maxDepth +
    filterHeadersHeight
  );
}
