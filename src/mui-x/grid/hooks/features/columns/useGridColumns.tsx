...
export const columnsStateInitializer: GridStateInitializer<
  Pick<
    ...
    "columnVisibilityModel" | "initialState" | "columns"
  >
> = (state, props, apiRef) => {
...
export function useGridColumns(
  ...
  const getVisibleColumns = React.useCallback<
    GridColumnApi["getVisibleColumns"]
  >(() => gridVisibleColumnDefinitionsSelector(apiRef), [apiRef]);
  ...
  const getColumnPosition = React.useCallback<
    GridColumnApi["getColumnPosition"]
  >(
    (field) => {
  ...
  const setColumnVisibilityModel = React.useCallback<
    GridColumnApi["setColumnVisibilityModel"]
  >(
    (model) => {
  ...
  const setColumnVisibility = React.useCallback<
    GridColumnApi["setColumnVisibility"]
  >(
    (field, isVisible) => {
  ...
  const setColumnIndex = React.useCallback<
    GridColumnReorderApi["setColumnIndex"]
  >(
    (field, targetIndexPosition) => {
      ...
      const params: GridColumnOrderChangeParams = {
        ...
        targetIndex:
          apiRef.current.getColumnIndexRelativeToVisibleColumns(field),
  ...
  const setColumnWidth = React.useCallback<GridColumnApi["setColumnWidth"]>(
    (field, width) => {
      ...
      const newColumn: GridStateColDef = {
        ...column,
        width,
        hasBeenResized: true,
      };
  ...
  >(
    (prevState, context) => {
      ...
      const shouldExportColumnVisibilityModel =
        ...
        Object.keys(props.initialState?.columns?.columnVisibilityModel ?? {})
          .length > 0 ||
