...
export const useGridFocus = (
  ...
  const publishCellFocusOut = React.useCallback(
    (
      ...
      event: GridEventLookup["cellFocusOut"]["event"],
    ) => {
      if (cell) {
  ...
  const setColumnHeaderFocus = React.useCallback<
    GridFocusApi["setColumnHeaderFocus"]
  >(
    (field, event = {}) => {
  ...
  const moveFocusToRelativeCell = React.useCallback<
    GridFocusPrivateApi["moveFocusToRelativeCell"]
  >(
    (id, field, direction) => {
      ...
      const rowToFocus: GridRowEntry | undefined =
        currentPageRows[rowIndexToFocus];
  ...
  const handleColumnHeaderFocus = React.useCallback<
    GridEventListener<"columnHeaderFocus">
  >(
    ({ field }, event) => {
  ...
  const handleBlur = React.useCallback<GridEventListener<"columnHeaderBlur">>(
    (_, event) => {
      if (
        event.relatedTarget
          ?.getAttribute("class")
          ?.includes(gridClasses.columnHeader)
      ) {
        return;
      }
  ...
  const handleDocumentClick = React.useCallback(
    (event: MouseEvent) => {
      ...
      if (
        cellParams?.id === focusedCell.id &&
        cellParams?.field === focusedCell.field
      ) {
        return;
      }
  ...
  const handleCellModeChange = React.useCallback<
    GridEventListener<"cellModeChange">
  >(
    (params) => {
  ...
  const handleRowSet = React.useCallback<GridEventListener<"rowsSet">>(() => {
    ...
    if (cell && !apiRef.current.getRow(cell.id)) {
      ...
      if (typeof lastFocusedRowId !== "undefined") {
        ...
        const lastFocusedRowIndex = rowEl?.dataset.rowindex
        ...
        const nextRow =
          currentPage.rows[
            clamp(lastFocusedRowIndex, 0, currentPage.rows.length - 1)
          ];
      ...
      apiRef.current.setState((state) => ({
        ...
        focus: {
          cell:
            nextRowId === null ? null : { id: nextRowId, field: cell.field },
          columnHeader: null,
          columnHeaderFilter: null,
          columnGroupHeader: null,
        },
  ...
  const handlePaginationModelChange = useEventCallback(() => {
    ...
    const rowIsInCurrentPage = currentPage.rows.find(
      (row) => row.id === currentFocusedCell.id,
    );
