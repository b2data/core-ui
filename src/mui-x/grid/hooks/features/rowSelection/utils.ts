...
function getGridRowGroupSelectableDescendants(
  ...
  for (
    ...
    rowTree[sortedRowIds[index]]?.depth > groupNode.depth;
    index += 1
  ) {
...
export const checkboxPropsSelector = createSelector(
  ...
  (
    ...
    rowSelectionManager,
    {
      groupId,
      autoSelectParents,
    }: { groupId: GridRowId; autoSelectParents: boolean },
  ) => {
    const groupNode = rowTree[groupId];
    if (
      !groupNode ||
...
export function isMultipleRowSelectionEnabled(
  ...
  if (props.signature === GridSignature.DataGrid) {
    ...
    return (
      props.checkboxSelection && props.disableMultipleRowSelection !== true
    );
  }
...
const getFilteredRowNodeSiblings = (
  ...
  return parentNode.children.filter(
    (childId) => childId !== id && filteredRows[childId] !== false,
  );
};
...
export const findRowsToSelect = (
  ...
  rowSelectionManager: RowSelectionManager = gridRowSelectionManagerSelector(
    apiRef,
  ),
) => {
  ...
  if (
    (!autoSelectDescendants && !autoSelectParents) ||
    filteredRows[selectedRow] === false
  ) {
    return;
  }
  ...
  if (autoSelectDescendants) {
    ...
    if (rowNode?.type === "group") {
      const descendants = getGridRowGroupSelectableDescendants(
        apiRef,
        selectedRow,
      );
  ...
  if (autoSelectParents) {
    ...
    const traverseParents = (rowId: GridRowId) => {
      const siblings: GridRowId[] = getFilteredRowNodeSiblings(
        tree,
        filteredRows,
        rowId,
      );
      if (
        siblings.length === 0 ||
        siblings.every(checkAllDescendantsSelected)
      ) {
