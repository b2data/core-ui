...
> = (state, props) => ({
  ...
  rowSelection: props.rowSelection
    ? (props.rowSelectionModel ?? emptyModel)
    : emptyModel,
});
...
export const useGridRowSelection = (
  ...
  const setRowSelectionModel = React.useCallback<
    GridRowSelectionApi["setRowSelectionModel"]
  >(
    (model) => {
      if (
        props.signature === GridSignature.DataGrid &&
  ...
  const isRowSelectable = React.useCallback<
    GridRowSelectionApi["isRowSelectable"]
  >(
    (id) => {
  ...
  const getSelectedRows = React.useCallback<
    GridRowSelectionApi["getSelectedRows"]
    ...
    (ids: GridRowId[], isSelected = true, resetSelection = false) => {
      ...
      if (resetSelection) {
        ...
        if (
          ...
          Array.from(newSelectionModel.ids).every((id) =>
            currentSelectionModel.ids.has(id),
          )
        ) {
          return;
        }
  ...
  >(
    (inputSelectionModel) => {
      if (
        !isNestedData ||
        !applyAutoSelection ||
        (inputSelectionModel.ids.size === 0 &&
          inputSelectionModel.type === "include")
      ) {
      ...
      const selectionManager = createRowSelectionManager(
  ...
  const selectRowRange = React.useCallback<
    GridRowMultiSelectionApi["selectRowRange"]
  >(
    (
      {
        startId,
  ...
  const handleSingleRowSelection = React.useCallback(
    (id: GridRowId, event: React.MouseEvent | React.KeyboardEvent) => {
      ...
      const resetSelection =
        !canHaveMultipleSelection || isMultipleSelectionDisabled;
  ...
  const toggleAllRows = React.useCallback(
    (value: boolean) => {
      ...
      const hasFilters =
        filterModel.items.length > 0 || (quickFilterModel?.length || 0) > 0;
      if (
  ...
  const handleCellKeyDown = React.useCallback<GridEventListener<"cellKeyDown">>(
    (params, event) => {
      ...
      if (
        apiRef.current.getCellMode(params.id, params.field) ===
        GridCellModes.Edit
      ) {
        return;
      }
      ...
      if (isNavigationKey(event.key) && event.shiftKey) {
        ...
        if (focusCell && focusCell.id !== params.id) {
          ...
          const newRowIndex = apiRef.current.getRowIndexRelativeToVisibleRows(
            focusCell.id,
          );
  ...
  const syncControlledState = useEventCallback(() => {
    ...
    if (
      !applyAutoSelection ||
      !isNestedData ||
      (propRowSelectionModel.type === "include" &&
    ...
    const newSelectionModel = apiRef.current.getPropagatedRowSelectionModel(
    ...
    if (
      ...
      !Array.from(propRowSelectionModel.ids).every((id) =>
        newSelectionModel.ids.has(id),
      )
    ) {
      apiRef.current.setRowSelectionModel(newSelectionModel);
      return;
  ...
  useGridEvent(
    apiRef,
    "cellKeyDown",
    runIfRowSelectionIsEnabled(handleCellKeyDown),
  );
