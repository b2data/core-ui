...
> = (state, props) => {
  const sortModel =
    props.sortModel ?? props.initialState?.sorting?.sortModel ?? [];
...
export const useGridSorting = (
  ...
  const createSortItem = React.useCallback(
    (
      col: GridColDef,
      directionOverride?: GridSortDirection,
      ...
      if (existing) {
        const nextSort =
          directionOverride === undefined
            ? getNextGridSortDirection(
                col.sortingOrder ?? props.sortingOrder,
                existing.sort,
              )
            : directionOverride;
  ...
  const getRowIdFromRowIndex = React.useCallback<
    GridSortApi["getRowIdFromRowIndex"]
  >((index) => apiRef.current.getSortedRowIds()[index], [apiRef]);
  ...
  const flatSortingMethod = React.useCallback<GridStrategyProcessor<"sorting">>(
    (params) => {
      ...
      const sortedChildren = params.sortRowList
        ? params.sortRowList(
            rootGroupNode.children.map((childId) => rowTree[childId]),
          )
