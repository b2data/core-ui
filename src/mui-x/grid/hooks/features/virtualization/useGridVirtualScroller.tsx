...
export const EMPTY_DETAIL_PANELS = Object.freeze(
...
export const useGridVirtualScroller = () => {
  ...
  const enabledForRows =
    useGridSelector(apiRef, gridVirtualizationRowEnabledSelector) && !isJSDOM;
  const enabledForColumns =
    useGridSelector(apiRef, gridVirtualizationColumnEnabledSelector) &&
    !isJSDOM;
  ...
  const isRtl = useRtl();
  const rowSelectionManager = useGridSelector(
    apiRef,
  ...
  const needsHorizontalScrollbar = useGridSelector(
    apiRef,
    needsHorizontalScrollbarSelector,
  );
  const verticalScrollbarWidth = useGridSelector(
    apiRef,
    gridVerticalScrollbarWidthSelector,
  );
  ...
  const scrollPosition = React.useRef(
    rootProps.initialState?.scroll ?? EMPTY_SCROLL_POSITION,
  );
    ...
    (nextRenderContext: GridRenderContext) => {
      if (
        areRenderContextsEqual(
          nextRenderContext,
          apiRef.current.state.virtualization.renderContext,
        )
      ) {
        return;
      }
      ...
      const didRowsIntervalChange =
        nextRenderContext.firstRowIndex !==
          previousRowContext.current.firstRowIndex ||
        nextRenderContext.lastRowIndex !==
          previousRowContext.current.lastRowIndex;
  ...
  const triggerUpdateRenderContext = useEventCallback(() => {
    ...
    const didCrossThreshold =
      rowScroll >= rowHeight || columnScroll >= MINIMUM_COLUMN_WIDTH;
  ...
  const forceUpdateRenderContext = () => {
    ...
    if (
      !gridDimensionsSelector(apiRef).isReady &&
  ...
  const getRows = (
    ...
    const lastRowToRender = Math.min(
      baseRenderContext.lastRowIndex,
      rowModels.length,
    );
    ...
    rowIndexes.forEach((rowIndexInPage) => {
      ...
      if (hasColSpan) {
        ...
        const maxLastColumn =
          visibleColumns.length - pinnedColumns.right.length;
      ...
      const showBottomBorder =
        isLastVisibleInSection && params.position === "top";
      ...
      rows.push(
        <rootProps.slots.row
          ...
          focusedColumnIndex={
            isVirtualFocusColumn ? focusedVirtualCell!.columnIndex : undefined
          }
      ...
      if (params.position === undefined && isLastVisibleInSection) {
        rows.push(
          apiRef.current.getInfiniteLoadingTriggerElement?.({ lastRowId: id }),
        );
      }
    });
    return rows;
...
type RenderContextInputs = {
  ...
  hiddenCellsOriginMap: ReturnType<
    typeof gridRowSpanningHiddenCellsOriginMapSelector
  >;
  listView: boolean;
...
function inputsSelector(
  ...
  const hiddenCellsOriginMap =
    gridRowSpanningHiddenCellsOriginMapSelector(apiRef);
  ...
  return {
    ...
    lastRowHeight:
      lastRowId !== undefined
        ? apiRef.current.unstable_getRowHeight(lastRowId)
        : 0,
  ...
  if (inputs.enabledForRows) {
    ...
    let firstRowIndex = Math.min(
      getNearestIndexToRender(inputs, top, {
        atStart: true,
        lastPosition:
          inputs.rowsMeta.positions[inputs.rowsMeta.positions.length - 1] +
          inputs.lastRowHeight,
      }),
    ...
    if (rowSpanHiddenCellOrigin) {
      const minSpannedRowIndex = Math.min(
        ...Object.values(rowSpanHiddenCellOrigin),
      );
...
function getNearestIndexToRender(
  ...
  const lastMeasuredIndexRelativeToAllRows =
    inputs.apiRef.current.getLastMeasuredRowIndex();
  let allRowsMeasured = lastMeasuredIndexRelativeToAllRows === Infinity;
  if (inputs.range?.lastRowIndex && !allRowsMeasured) {
    // Check if all rows in this page are already measured
    allRowsMeasured =
      lastMeasuredIndexRelativeToAllRows >= inputs.range.lastRowIndex;
  }
...
function deriveRenderContext(
  ...
  const [initialFirstColumnToRender, lastColumnToRender] = getIndexesToRender({
    ...
    maxLastIndex:
      inputs.visibleColumns.length - inputs.pinnedColumns.right.length,
...
function binarySearch(
  ...
  if (options?.atStart) {
    const width =
      (pivot === positions.length - 1
        ? options.lastPosition
        : positions[pivot + 1]) - position;
