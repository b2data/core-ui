...
export const useGridColumnMenu = (
  ...
  const showColumnMenu = React.useCallback<GridColumnMenuApi["showColumnMenu"]>(
    (field) => {
      ...
      const shouldUpdate =
        newState.open !== columnMenuState.open ||
        newState.field !== columnMenuState.field;
  ...
  const hideColumnMenu = React.useCallback<
    GridColumnMenuApi["hideColumnMenu"]
  >(() => {
    const columnMenuState = gridColumnMenuSelector(apiRef);
    ...
    if (columnMenuState.field) {
      ...
      if (columnVisibilityModel[fieldToFocus] === false) {
        ...
        fieldToFocus =
          visibleOrderedFields[fieldIndex + 1] ||
          visibleOrderedFields[fieldIndex - 1];
      }
  ...
  const toggleColumnMenu = React.useCallback<
    GridColumnMenuApi["toggleColumnMenu"]
  >(
    (field) => {
