...
type CSVOptions = Required<
  Pick<
    GridCsvExportOptions,
    "delimiter" | "shouldAppendQuotes" | "escapeFormulas"
  >
>;
...
export function buildCSV(options: BuildCSVOptions): string {
  ...
  if (csvOptions.includeColumnGroupsHeaders) {
    ...
    >((acc, column) => {
      ...
      maxColumnGroupsDepth = Math.max(
        maxColumnGroupsDepth,
        columnGroupPath.length,
      );
      return acc;
    }, {});
    ...
    for (let i = 0; i < maxColumnGroupsDepth; i += 1) {
      ...
      filteredColumns.forEach((column) => {
        ...
        headerGroupRow.addValue(
          columnGroup ? columnGroup.headerName || columnGroup.groupId : "",
        );
      });
    }
  }
