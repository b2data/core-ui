...
export const mergeStateWithFilterModel =
  ...
  (
    filteringState: GridStateCommunity["filter"],
  ): GridStateCommunity["filter"] => ({
    ...filteringState,
...
const getFilterCallbackFromItem = (
  ...
  if (!filterOperators?.length) {
    throw new Error(
      `MUI X: No filter operators found for column '${column.field}'.`,
    );
  }
...
return result$$;`.replaceAll("$$", String(filterItemsApplierId)),
  ...
  const filterItem: GridFilterItemApplierNotAggregated = (
    row,
    shouldApplyItem,
...
export const shouldQuickFilterExcludeHiddenColumns = (
  filterModel: GridFilterModel,
) => {
...
const buildAggregatedQuickFilterApplier = (
  ...
  const quickFilterValues =
    filterModel.quickFilterValues?.filter(Boolean) ?? [];
  ...
  if (shouldQuickFilterExcludeHiddenColumns(filterModel)) {
    ...
    columnFields = allColumnFields.filter(
...
export const buildAggregatedFilterApplier = (
  ...
  return function isRowMatchingFilters(row, shouldApplyFilter, result) {
    result.passingFilterItems =
      isRowMatchingFilterItems?.(row, shouldApplyFilter) ?? null;
    result.passingQuickFilterValues =
      isRowMatchingQuickFilter?.(row, shouldApplyFilter) ?? null;
  };
};
...
export const passFilterLogic = (
  ...
  if (cleanedFilterItemResults.length > 0) {
    ...
    const filterItemPredicate = (item: GridFilterItem) => {
      return cleanedFilterItemResults.some(
        (filterItemResult) => filterItemResult[item.id!],
      );
    };

    const logicOperator =
      filterModel.logicOperator ?? getDefaultGridFilterModel().logicOperator;
  ...
  if (
    cleanedQuickFilterResults.length > 0 &&
    ...
    const quickFilterLogicOperator =
      ...
      getDefaultGridFilterModel().quickFilterLogicOperator;
    if (quickFilterLogicOperator === GridLogicOperator.And) {
      const passesAllQuickFilterValues = filterModel.quickFilterValues.every(
      ...
      const passesSomeQuickFilterValues = filterModel.quickFilterValues.some(
        quickFilterValuePredicate,
      );
