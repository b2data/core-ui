...
const gridFilterStateSelector = createRootSelector(
  (state: GridStateCommunity) => state.filter,
);
 ...
 * @ignore - do not document.
 */
export const gridExpandedSortedRowTreeLevelPositionLookupSelector =
  createSelectorMemoized(
    ...
    (visibleSortedRowIds, rowTree) => {
      ...
      return visibleSortedRowIds.reduce(
        (acc: Record<GridRowId, number>, rowId) => {
...
export const gridFilterActiveItemsLookupSelector = createSelectorMemoized(
  ...
  (activeFilters) => {
    const result: GridFilterActiveItemsLookup =
      activeFilters.reduce<GridFilterActiveItemsLookup>((res, filterItem) => {
...
export const gridExpandedSortedRowIndexLookupSelector = createSelectorMemoized(
  gridExpandedSortedRowIdsSelector,
  (expandedSortedIds) => {
    return expandedSortedIds.reduce<Record<GridRowId, number>>(
      (acc, id, index) => {
        acc[id] = index;
        return acc;
      },
