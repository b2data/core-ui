...
export const useGridColumnHeaders = (props: UseGridColumnHeadersProps) => {
  ...
  const columnGroupsModel = useGridSelector(
    apiRef,
    gridColumnGroupsUnwrappedModelSelector,
  );
  const columnPositions = useGridSelector(apiRef, gridColumnPositionsSelector);
  const renderContext = useGridSelector(
    apiRef,
  ...
  const groupHeaderHeight = useGridSelector(
    apiRef,
    gridGroupHeaderHeightSelector,
  );
  const scrollbarWidth = useGridSelector(
    apiRef,
    gridVerticalScrollbarWidthSelector,
  );
  ...
  const getColumnGroupHeaders = ({
    ...
    const firstGroupToRender =
      columnGroupsModel[firstColumnFieldToRender]?.[depth] ?? null;
    ...
    const lastGroupToRender =
      columnGroupsModel[lastColumnFieldToRender]?.[depth] ?? null;
      ...
      ({ groupId, columnFields }, index) => {
        ...
        const headerInfo: HeaderInfo = {
          ...
          width: columnFields.reduce(
            (acc, field) => acc + columnsLookup[field].computedWidth,
            0,
          ),
          fields: columnFields,
          colIndex: columnIndex,
