...
function getResizeDirection(separator: HTMLElement, isRtl: boolean) {
  const side = separator.classList.contains(
...
export const useGridColumnResize = (
  ...
  const finishResize = (nativeEvent: MouseEvent) => {
    ...
    if (refs.colDef) {
      ...
      logger.debug(
        `Updating col ${refs.colDef.field} with new width: ${refs.colDef.width}`,
      );
  ...
  const storeReferences = (
    ...
    refs.cellElements = findGridCellElementsFromCol(
    ...
    async (userOptions) => {
      ...
      try {
        if (
          ...
          options.disableColumnVirtualization
        ) {
...
function updateProperty(
  ...
  element.style[property] =
    `${parseInt(element.style[property], 10) + delta}px`;
}
