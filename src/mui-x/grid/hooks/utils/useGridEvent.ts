...
export function useGridEvent<Api extends GridApiCommon, E extends GridEvents>(
  ...
  const objectRetainedByReact = React.useState(
    ObjectToBeRetainedByReact.create,
  )[0];
  ...
  React.useEffect(() => {
    if (!subscription.current && handlerRef.current) {
      const enhancedHandler: GridEventListener<E> = (
        params,
        event,
        details,
      ) => {
...
export function useGridEventPriority<
  Api extends GridApiCommon,
  E extends GridEvents,
>(apiRef: RefObject<Api>, eventName: E, handler?: GridEventListener<E>) {
