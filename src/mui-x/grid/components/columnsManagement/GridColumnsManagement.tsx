...
function GridColumnsManagement(props: GridColumnsManagementProps) {
  ...
  const pivotInitialColumns = useGridSelector(
    apiRef,
    gridPivotInitialColumnsSelector,
  );
  const columns = React.useMemo(
    () =>
      pivotActive
        ? Array.from(pivotInitialColumns.values())
        : columnDefinitions,
  ...
  const currentColumns = React.useMemo(() => {
    const togglableColumns = getTogglableColumns
      ? getTogglableColumns(sortedColumns)
      : null;
    ...
    (isVisible: boolean) => {
      ...
      (toggleAllMode === "filteredOnly" ? currentColumns : columns).forEach(
        (col) => {
          if (
            col.hideable &&
            (togglableColumns == null || togglableColumns.includes(col.field))
          ) {
            if (isVisible) {
        ...
        >
          {!disableShowHideToggle ? (
            <rootProps.slots.baseCheckbox
              ...
              indeterminate={
                !allHideableColumnsVisible && !allHideableColumnsHidden
              }
              onChange={() => toggleAllColumns(!allHideableColumnsVisible)}
              name={apiRef.current.getLocaleText(
...
const GridColumnsManagementRow = styled(
  NotRendered<GridSlotProps["baseCheckbox"]>,
  {
    name: "MuiDataGrid",
