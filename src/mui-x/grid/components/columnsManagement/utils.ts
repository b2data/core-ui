...
export const checkColumnVisibilityModelsSame = (
  ...
  const aFalseValues = new Set(
    Object.keys(a).filter((key) => a[key] === false),
  );
  const bFalseValues = new Set(
    Object.keys(b).filter((key) => b[key] === false),
  );
...
export const defaultSearchPredicate: NonNullable<
  GridColumnsManagementProps["searchPredicate"]
> = (column, searchValue) =>
