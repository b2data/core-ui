...
type GridSkeletonLoadingOverlayInnerProps =
...
const getColIndex = (el: HTMLElement) =>
...
>(function GridSkeletonLoadingOverlayInner(props, forwardedRef) {
  ...
  const { skeletonRowsCount, visibleColumns, showFirstRowBorder, ...rest } =
    props;
  const allVisibleColumns = useGridSelector(
    apiRef,
    gridVisibleColumnDefinitionsSelector,
  );
      ...
      for (let colIndex = 0; colIndex < columns.length; colIndex += 1) {
        ...
        const sectionLength = pinnedSide
          ...
          : columns.length -
            ...
            pinnedColumns.right.length; // middle section
        const sectionIndex = pinnedSide
          ? pinnedColumns[pinnedSide].findIndex(
              (col) => col.field === column.field,
            ) // pinned section
          : colIndex - pinnedColumns.left.length; // middle section
        const scrollbarWidth = dimensions.hasScrollY
        ...
        const hasFillerAfter =
          isLastColumn && !isFirstPinnedRight && gridHasFiller;
        const expandedWidth =
          dimensions.viewportOuterSize.width - dimensions.columnsTotalWidth;
  ...
  const handleColumnResize: GridEventListener<"columnResize"> = (params) => {
    ...
    const resizedColIndex = columns.findIndex(
