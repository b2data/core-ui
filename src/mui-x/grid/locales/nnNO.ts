...
const nnNOGrid: Partial<GridLocaleText> = {
  ...
  footerRowSelected: (count) =>
    count !== 1
      ? `${count.toLocaleString()} rader valt`
      : `${count.toLocaleString()} rad valt`,
  ...
  paginationDisplayedRows: ({ from, to, count, estimated }) => {
    ...
    const estimatedLabel =
      estimated && estimated > to ? `omtrent ${estimated}` : `flere enn ${to}`;
  ...
  promptFieldSpeechRecognitionNotSupported:
    "Talegjenkjenning er ikkje støtta i denne nettlesaren",
