  ...
  >(
    (columnsState) => {
      ...
      const hasToggleColumn =
        columnsState.lookup[GRID_DETAIL_PANEL_TOGGLE_FIELD] != null;
      ...
      if (shouldHaveToggleColumn && !hasToggleColumn) {
        ...
        if (
          !props.columns.some(
            (col) => col.field === GRID_DETAIL_PANEL_TOGGLE_FIELD,
          )
        ) {
          columnsState.orderedFields = [
            GRID_DETAIL_PANEL_TOGGLE_FIELD,
  ...
  >(
    (classes, id) => {
      ...
      const expandedRowIds =
        gridDetailPanelExpandedRowIdsSelector(privateApiRef);
