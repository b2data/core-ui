...
export const useGridRowReorder = (
  ...
  const sortedRowIndexLookup = useGridSelector(
    apiRef,
    gridExpandedSortedRowIndexLookupSelector,
  );
  ...
  const previousReorderState =
    React.useRef<ReorderStateProps>(EMPTY_REORDER_STATE);
  ...
  const applyRowAnimation = React.useCallback(
    (callback: () => void) => {
      ...
      requestAnimationFrame(() => {
        const newRows = rootElement.querySelectorAll(
          "[data-id]",
  ...
  >(
    (params, event) => {
      ...
      if (
        ...
        timeout.start(500, () => {
          const rowNode = gridRowNodeSelector(
            apiRef,
            params.id,
          ) as GridGroupNode;
      ...
      if (
        ...
        const validatedIndex = apiRef.current.unstable_applyPipeProcessors(
          ...
          -1,
          {
            ...
            dropPosition,
            dragDirection:
              currentReorderState.dragDirection as GridRowReorderDirection,
          },
        );
  ...
  const handleDragEnd = React.useCallback<GridEventListener<"rowDragEnd">>(
    (_, event): void => {
      ...
      if (
        dropTarget.current.targetRowIndex !== null &&
        dropTarget.current.targetRowId !== null
      ) {
  ...
  const getRowReorderTargetIndex = React.useCallback<
    GridPipeProcessor<"getRowReorderTargetIndex">
  >(
    (
      initialValue,
      { sourceRowId, targetRowId, dropPosition, dragDirection },
    ) => {
      ...
      if (dragDirection === "up") {
        finalTargetIndex =
          dropPosition === "above" ? targetRowIndex : targetRowIndex + 1;
      } else {
        finalTargetIndex =
          dropPosition === "above" ? targetRowIndex - 1 : targetRowIndex;
      }
