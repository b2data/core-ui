...
export const useGridDataSourceLazyLoader = (
  ...
  const [
    lazyLoadingRowsUpdateStrategyActive,
    ...
    (params: GridGetRowsParams) => {
      ...
      return {
        ...
        start: params.start - (params.start % paginationModel.pageSize),
          ...
          (params.end % paginationModel.pageSize) -
          1,
      };
    },
    [privateApiRef],
  ...
  const ensureValidRowCount = React.useCallback(
    (
      previousLoadingTrigger: LoadingTrigger,
      newLoadingTrigger: LoadingTrigger,
    ) => {
  ...
  const addSkeletonRows = React.useCallback(() => {
    ...
    const dataRowIdToModelLookup =
      privateApiRef.current.state.rows.dataRowIdToModelLookup;
    const rootGroup = tree[GRID_ROOT_GROUP_ID] as GridGroupNode;
    const rootGroupChildren = [...rootGroup.children];
  ...
  const handleDataUpdate = React.useCallback<
    GridStrategyProcessor<"dataSourceRowsUpdate">
  >(
    (params) => {
      if ("error" in params) {
        return;
      ...
      if (response.rowCount !== undefined || pageRowCount === undefined) {
        privateApiRef.current.setRowCount(
          response.rowCount === undefined ? -1 : response.rowCount,
        );
      }
      ...
      if (rowsStale.current && params.fetchParams.start === 0) {
        ...
        const filteredSortedRowIds =
          gridFilteredSortedRowIdsSelector(privateApiRef);
        ...
        privateApiRef.current.unstable_replaceRows(
          startingIndex,
          response.rows,
        );
      }
  ...
  const handleScrolling: GridEventListener<"scrollPositionChange"> =
    React.useCallback(
      (newScrollPosition) => {
        ...
        const position =
          newScrollPosition.top + dimensions.viewportInnerSize.height;
  ...
  const throttledHandleRenderedRowsIntervalChange = React.useMemo(
    () =>
      throttle(
        ...
        props.lazyLoadingRequestThrottleMs,
      ),
