...
export const useGridDataSourceBasePro = <Api extends GridPrivateApiPro>(
  ...
  const { api, strategyProcessor, events, cacheChunkManager, cache } =
  ...
  const fetchRowChildren = React.useCallback<
    GridDataSourcePrivateApiPro["fetchRowChildren"]
  >(
    async (id) => {
      ...
      const cachedData = responses.some((response) => response === undefined)
        ? undefined
        : CacheChunkManager.mergeResponses(
            responses as GridGetRowsResponsePro[],
          );
      ...
      if (cachedData !== undefined) {
        ...
        apiRef.current.setRowCount(
          cachedData.rowCount === undefined ? -1 : cachedData.rowCount,
        );
      ...
      try {
        ...
        apiRef.current.setRowCount(
          getRowsResponse.rowCount === undefined
            ? -1
            : getRowsResponse.rowCount,
        );
  ...
  const setChildrenLoading = React.useCallback<
    GridDataSourceApiBasePro["setChildrenLoading"]
  >(
    (parentId, isLoading) => {
