...
const selectors = {
  renderContext: createSelector(
    (state: BaseState) => state.virtualization.renderContext,
  ),
  enabledForRows: createSelector(
    (state: BaseState) => state.virtualization.enabledForRows,
  ),
  enabledForColumns: createSelector(
    (state: BaseState) => state.virtualization.enabledForColumns,
  ),
};
...
function useVirtualization(
  ...
  const needsHorizontalScrollbar = useStore(
    store,
  ...
  const scrollPosition = React.useRef(
    initialState?.scroll ?? EMPTY_SCROLL_POSITION,
  );
    ...
    (nextRenderContext: RenderContext) => {
      if (
        !areRenderContextsEqual(
          nextRenderContext,
          store.state.virtualization.renderContext,
        )
      ) {
  ...
  const triggerUpdateRenderContext = useEventCallback(() => {
    ...
    const direction = isScrolling
      ? ScrollDirection.forDelta(dx, dy)
  ...
  const getRows = (
    ...
    const columnPositions = Dimensions.selectors.columnPositions(
      store.state,
      columns,
    );
    ...
    rowIndexes.forEach((rowIndexInPage) => {
      ...
      const rowIndex =
        (range?.firstRowIndex || 0) + rowIndexOffset + rowIndexInPage;
      ...
      const showBottomBorder =
        isLastVisibleInSection && rowParams.position === "top";
      ...
      rowElements.push(
        renderRow({
          ...
          firstColumnIndex,
          lastColumnIndex,
          focusedColumnIndex: isVirtualFocusColumn
            ? focusedVirtualCell!.columnIndex
            : undefined,
          isFirstVisible,
          isLastVisible,
          isVirtualFocusRow,
...
function inputsSelector(
  ...
  return {
    ...
    lastRowHeight:
      lastRowId !== undefined ? api.rowsMeta.getRowHeight(lastRowId) : 0,
    lastColumnWidth: lastColumn?.computedWidth ?? 0,
    rowsMeta: Dimensions.selectors.rowsMeta(store.state),
    columnPositions: Dimensions.selectors.columnPositions(
      store.state,
      params.columns,
    ),
    rows,
    ...
    virtualizeColumnsWithAutoRowHeight:
      params.virtualizeColumnsWithAutoRowHeight,
  };
}
  ...
  if (inputs.enabledForRows) {
    ...
    if (rowSpanHiddenCellOrigin) {
      const minSpannedRowIndex = Math.min(
        ...(Object.values(rowSpanHiddenCellOrigin) as any),
      );
...
function getNearestIndexToRender(
  ...
  const lastMeasuredIndexRelativeToAllRows =
    inputs.api.rowsMeta.getLastMeasuredRowIndex();
...
function deriveRenderContext(
  ...
  const [initialFirstColumnToRender, lastColumnToRender] = getIndexesToRender({
    ...
    maxLastIndex:
      inputs.columns.length - (inputs.pinnedColumns?.right.length ?? 0),
  ...
  while (!foundStableColumn && firstNonSpannedColumnToRender >= 0) {
    ...
    for (let i = firstRowToRender; i < lastRowToRender; i += 1) {
      ...
      if (row) {
        ...
        const cellColSpanInfo = api.getCellColSpanInfo(
          rowId,
