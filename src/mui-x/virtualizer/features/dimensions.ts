...
const selectors = {
  ...
  needsHorizontalScrollbar: (state: BaseState) =>
    ...
    state.dimensions.columnsTotalWidth >
      state.dimensions.viewportOuterSize.width,
};
...
function useDimensions(
  ...
  const updateDimensions = React.useCallback(() => {
    ...
    const topContainerHeight =
      topPinnedHeight + rowsMeta.pinnedTopRowsTotalHeight;
    const bottomContainerHeight =
      bottomPinnedHeight + rowsMeta.pinnedBottomRowsTotalHeight;
    ...
    if (params.autoHeight) {
      ...
      viewportInnerSize = {
        width: Math.max(
          0,
          viewportOuterSize.width - (hasScrollY ? scrollbarSize : 0),
        ),
        height: Math.max(
          0,
          viewportOuterSize.height - (hasScrollX ? scrollbarSize : 0),
        ),
      };
    } else {
      viewportOuterSize = {
      ...
      if (hasScrollXIfNoYScrollBar || hasScrollYIfNoXScrollBar) {
        ...
        hasScrollX =
          content.width + (hasScrollY ? scrollbarSize : 0) > container.width;
  ...
  const debouncedUpdateDimensions = React.useMemo(
    () =>
      resizeThrottleMs > 0
        ? throttle(updateDimensionCallback, resizeThrottleMs)
        : undefined,
  ...
  const hydrateRowsMeta = React.useCallback(() => {
    ...
    const didHeightsChange =
      pinnedTopRowsTotalHeight !==
        store.state.rowsMeta.pinnedTopRowsTotalHeight ||
      pinnedBottomRowsTotalHeight !==
        store.state.rowsMeta.pinnedBottomRowsTotalHeight ||
...
function observeRootNode(
  ...
  if (
    store.state.rootSize === Size.EMPTY ||
    !Size.equals(initialSize, store.state.rootSize)
  ) {
