...
export const useTreeViewItems: TreeViewPlugin<UseTreeViewItemsSignature> = ({
  ...
  const setTreeViewLoading = useEventCallback((isLoading: boolean) => {
    store.set("items", { ...store.state.items, loading: isLoading });
  });
  const setTreeViewError = useEventCallback((error: Error | null) => {
    store.set("items", { ...store.state.items, error });
  });
  ...
  const getItemTree = React.useCallback(() => {
    const getItemFromItemId = (itemId: TreeViewItemId): TreeViewBaseItem => {
      ...
      const newChildren = itemsSelectors.itemOrderedChildrenIds(
        store.state,
        itemId,
      );
    ...
    return itemsSelectors
      .itemOrderedChildrenIds(store.state, null)
      .map(getItemFromItemId);
  }, [store]);

  const getItemOrderedChildrenIds = React.useCallback(
    (itemId: string | null) =>
      itemsSelectors.itemOrderedChildrenIds(store.state, itemId),
    [store],
  );
    ...
    if (items) {
      ...
      if (parentId) {
        newItems = {
          ...
          itemMetaLookup: {
            ...store.state.items.itemMetaLookup,
  ...
  const removeChildren = (parentId: string | undefined) => {
    if (parentId == null) {
      ...
      const newMetaMap = Object.keys(store.state.items.itemMetaLookup).reduce(
        (acc, key) => {
