...
> = ({ store, params }) => {
  ...
  const setItemSelection: UseTreeViewSelectionInstance["setItemSelection"] = ({
    ...
    if (keepExistingSelection) {
      ...
      const isSelectedBefore = selectionSelectors.isItemSelected(
        store.state,
        itemId,
      );
      if (
      ...
      if (
        ...
        (shouldBeSelected == null &&
          selectionSelectors.isItemSelected(store.state, itemId))
      ) {
  ...
  const selectRange = (
    ...
    let newSelectedItems = selectionSelectors
      .selectedItems(store.state)
      .slice();
  ...
  const selectRangeFromStartToItem = (
    event: React.SyntheticEvent,
    itemId: string,
  ) => {
  ...
  const selectRangeFromItemToEnd = (
    event: React.SyntheticEvent,
    itemId: string,
  ) => {
