...
export const getAddedAndRemovedItems = ({
  ...
  return {
    added: newModel.filter(
      (itemId) => !selectionSelectors.isItemSelected(store.state, itemId),
    ),
  ...
  changes.added.forEach((addedItemId) => {
    if (selectionPropagation.descendants) {
      const selectDescendants = (itemId: TreeViewItemId) => {
        ...
        itemsSelectors
          .itemOrderedChildrenIds(store.state, itemId)
    ...
    if (selectionPropagation.parents) {
      const checkAllDescendantsSelected = (itemId: TreeViewItemId): boolean => {
        ...
        const children = itemsSelectors.itemOrderedChildrenIds(
          store.state,
          itemId,
        );
