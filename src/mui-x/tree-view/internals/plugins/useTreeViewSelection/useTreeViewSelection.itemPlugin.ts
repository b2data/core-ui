...
const selectorCheckboxSelectionStatus = createSelector(
  (
    state: TreeViewState<
      [UseTreeViewItemsSignature, UseTreeViewSelectionSignature]
    >,
    ...
    const traverseDescendants = (itemToTraverseId: TreeViewItemId) => {
      ...
      itemsSelectors
        .itemOrderedChildrenIds(state, itemToTraverseId)
    ...
    const shouldSelectBasedOnDescendants =
      selectionSelectors.propagationRules(state).parents;
    return shouldSelectBasedOnDescendants &&
...
export const useTreeViewSelectionItemPlugin: TreeViewItemPlugin = ({
  ...
  const isCheckboxSelectionEnabled = useStore(
    store,
    selectionSelectors.isCheckboxSelectionEnabled,
  );
  const isItemSelectionEnabled = useStore(
    store,
    selectionSelectors.canItemBeSelected,
    itemId,
  );
  const checkboxSelectionStatus = useStore(
    store,
    selectorCheckboxSelectionStatus,
    itemId,
  );
