...
> = ({ instance, store, params }) => {
  ...
  const applyItemExpansion: UseTreeViewExpansionInstance["applyItemExpansion"] =
    useEventCallback(({ itemId, event, shouldBeExpanded }) => {
  ...
  const setItemExpansion: UseTreeViewExpansionInstance["setItemExpansion"] =
    useEventCallback(({ itemId, event = null, shouldBeExpanded }) => {
  ...
  const isItemExpanded: UseTreeViewExpansionPublicAPI["isItemExpanded"] =
  ...
  const expandAllSiblings = (
    ...
    const siblings = itemsSelectors.itemOrderedChildrenIds(
      store.state,
      itemMeta.parentId,
    );
    ...
    const newExpanded = expansionSelectors
      .expandedItemsRaw(store.state)
      .concat(diff);
