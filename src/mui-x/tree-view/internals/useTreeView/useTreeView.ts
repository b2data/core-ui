...
function initializeInputApiRef<T>(
  inputApiRef: React.RefObject<Partial<T> | undefined>,
) {
...
>): UseTreeViewReturnValue<TSignatures> => {
  ...
  const { pluginParams, forwardedProps, apiRef } =
    useExtractPluginParamsFromProps<TSignatures, typeof props>({
      plugins,
      props,
    });

  const instance = useRefWithInit(
    () => ({}) as TreeViewInstance<TSignatures>,
  ).current;
  const publicAPI =
    useTreeViewApiInitialization<TreeViewPublicAPI<TSignatures>>(apiRef);
  ...
  const store = useRefWithInit(() => {
    ...
    return new Store<TreeViewState<TSignaturesWithCorePluginSignatures>>(
      initialState,
    );
  }).current;
