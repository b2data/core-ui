...
const getLastNavigableItemInArray = (
  ...
  while (
    itemIndex >= 0 &&
    !itemsSelectors.canItemBeFocused(state, items[itemIndex])
  ) {
    itemIndex -= 1;
  }
...
export const getPreviousNavigableItem = (
  ...
  while (
    expansionSelectors.isItemExpanded(state, currentItemId) &&
...
export const findOrderInTremauxTree = (
  ...
  const ancestorFamily = itemsSelectors.itemOrderedChildrenIds(
    state,
    commonAncestor,
  );
...
export const getNonDisabledItemsInRange = (
  ...
  const getNextItem = (itemId: string) => {
    ...
    let itemMeta: TreeViewItemMeta | null = itemsSelectors.itemMeta(
      state,
      itemId,
    );
    while (itemMeta != null) {
      ...
      itemMeta = itemMeta.parentId
        ? itemsSelectors.itemMeta(state, itemMeta.parentId)
        : null;
    }
 ...
 */
export const isTargetInDescendants = (
  target: HTMLElement,
