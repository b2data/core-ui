...
>(
      ...
      if (
        ...
        (event.relatedTarget &&
          ...
          ((event.target &&
            ...
            (event.relatedTarget as HTMLElement)?.dataset?.element ===
              "labelInput"))
      ) {
        return;
      }
  ...
  const createLabelHandleDoubleClick =
    (otherHandlers: EventHandlers) =>
    (event: React.MouseEvent & TreeViewCancellableEvent) => {
  ...
  const getLoadingContainerProps = <
    ExternalProps extends Record<string, any> = {},
  >(
    externalProps: ExternalProps = {} as ExternalProps,
