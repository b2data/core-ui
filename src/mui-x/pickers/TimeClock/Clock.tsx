...
export function Clock(inProps: ClockProps) {
  ...
  const isPointerBetweenTwoClockValues =
    type === "hours" ? false : viewValue % 5 !== 0;
  ...
  const clampValue = (newValue: number) =>
    Math.max(minViewValue, Math.min(maxViewValue, newValue));

  const circleValue = (newValue: number) =>
    (newValue + (maxViewValue + 1)) % (maxViewValue + 1);
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    ...
    switch (event.key) {
      ...
      case "ArrowUp":
        handleValueChange(
          circleValue(viewValue + keyboardControlStep),
          "partial",
        );
        event.preventDefault();
        break;
