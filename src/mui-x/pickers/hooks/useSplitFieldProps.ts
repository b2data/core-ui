...
export type InternalPropNames<TValueType extends PickerValueType> =
  ...
  | (TValueType extends "date" | "date-time"
      ? (typeof DATE_VALIDATION_PROP_NAMES)[number]
      : never)
  | (TValueType extends "time" | "date-time"
      ? (typeof TIME_VALIDATION_PROP_NAMES)[number]
      : never)
  | (TValueType extends "date-time"
      ? (typeof DATE_TIME_VALIDATION_PROP_NAMES)[number]
      : never);
...
type ExtractInternalProps<
  ...
  TProps extends { [key in InternalPropNames<TValueType>]?: any },
> = {
  [K in keyof TProps]: K extends InternalPropNames<TValueType>
    ? TProps[K]
    : never;
};
