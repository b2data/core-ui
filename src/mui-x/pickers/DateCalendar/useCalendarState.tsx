  ...
  (
    ...
    switch (action.type) {
      ...
      case "changeMonthTimezone": {
        ...
        let newCurrentMonth = utils.setTimezone(
          state.currentMonth,
          newTimezone,
        );
        if (
          utils.getMonth(newCurrentMonth) !== utils.getMonth(state.currentMonth)
        ) {
          newCurrentMonth = utils.setMonth(
            newCurrentMonth,
            utils.getMonth(state.currentMonth),
          );
        }
        return {
          ...state,
...
export const useCalendarState = (
  ...
  const setVisibleDate = useEventCallback(
    ({ target, reason }: SetVisibleDateParameters) => {
      if (
      ...
      if (reason === "cell-interaction") {
        month = getCurrentMonthFromVisibleDate(
          target,
      ...
      const hasChangedMonth = !utils.isSameMonth(
        calendarState.currentMonth,
        month,
      );
      const hasChangedYear = !utils.isSameYear(
        calendarState.currentMonth,
        month,
      );
      if (hasChangedMonth) {
      ...
      dispatch({
        type: "setVisibleDate",
        month,
        direction: utils.isAfterDay(month, calendarState.currentMonth)
          ? "left"
          : "right",
        focusedDay:
          calendarState.focusedDay != null &&
