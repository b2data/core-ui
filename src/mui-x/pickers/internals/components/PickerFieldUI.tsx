...
import {
  useFieldOwnerState,
  UseFieldOwnerStateParameters,
} from "../hooks/useFieldOwnerState";
...
export const PickerFieldUIContext =
  React.createContext<PickerFieldUIContextValue>({
    slots: {},
    slotProps: {},
    inputRef: undefined,
  });
...
>(props: PickerFieldUIProps<TEnableAccessibleFieldDOMStructure, TProps>) {
  ...
  const handleClickOpeningButton = useEventCallback(
  ...
  const openPickerButtonPosition =
    triggerStatus !== "hidden" ? openPickerButtonPositionProp : null;

  const TextField =
    slots?.textField ??
    pickerFieldUIContext.slots.textField ??
    (fieldResponse.enableAccessibleFieldDOMStructure === false
      ? MuiTextField
      : PickersTextField);
  ...
  const InputAdornment =
    ...
    MuiInputAdornment;
  const {
    ownerState: startInputAdornmentOwnerState,
    ...startInputAdornmentProps
  } = useSlotProps({
  ...
  const { ownerState: endInputAdornmentOwnerState, ...endInputAdornmentProps } =
    useSlotProps({
      elementType: InputAdornment,
  ...
  const OpenPickerButton =
    pickerFieldUIContext.slots.openPickerButton ?? MuiIconButton;
  ...
  const OpenPickerIcon =
    pickerFieldUIContext.slots.openPickerIcon ?? defaultOpenPickerIcon;
  ...
  const ClearButton =
    slots?.clearButton ??
    pickerFieldUIContext.slots.clearButton ??
    MuiIconButton;
  // We don't want to forward the `ownerState` to the `<IconButton />` component, see mui/material-ui#34056
  const { ownerState: clearButtonOwnerState, ...clearButtonProps } =
    useSlotProps({
      ...
      additionalProps: {
        ...
        edge:
          ...
          textFieldProps.variant !== "standard" &&
          clearButtonPosition !== openPickerButtonPosition
            ? clearButtonPosition
            : false,
      },
      ownerState,
  ...
  const ClearIcon =
    slots?.clearIcon ?? pickerFieldUIContext.slots.clearIcon ?? MuiClearIcon;
...
export interface PickerFieldUISlotPropsFromContext
...
export function mergeSlotProps<
  TProps extends {},
  TOwnerState extends FieldOwnerState,
>(
...
export function useFieldTextFieldProps<
  TProps extends UseFieldOwnerStateParameters & {
    inputProps?: {};
    InputProps?: {};
  },
>(parameters: UseFieldTextFieldPropsParameters) {
  ...
  const { InputProps, inputProps, ...otherExternalForwardedProps } =
...
export function PickerFieldUIContextProvider(
  props: PickerFieldUIContextProviderProps,
) {
  ...
  return (
    <PickerFieldUIContext.Provider value={contextValue}>
      {children}
    </PickerFieldUIContext.Provider>
  );
}
