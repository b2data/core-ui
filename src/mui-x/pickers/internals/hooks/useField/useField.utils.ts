...
export const removeLocalizedDigits = (
  valueStr: string,
...
export const applyLocalizedDigits = (
  valueStr: string,
  localizedDigits: string[],
) => {
  if (localizedDigits[0] === "0") {
...
export const doesSectionFormatHaveLeadingZeros = (
  ...
  switch (sectionType) {
    ...
    case "year": {
      ...
      return utils
        .formatByString(utils.setYear(now, 1), format)
        .startsWith("0");
    }
...
export const createDateStrForV7HiddenInputFromSections = (
  sections: FieldSection[],
) =>
  sections
    .map((section) => {
...
export const getSectionsBoundaries = (
  ...
  return {
    ...
    day: ({ currentDate }) => ({
      minimum: 1,
      maximum: utils.isValid(currentDate)
...
const transferDateSectionValue = (
  ...
  switch (section.type) {
    ...
    case "weekDay": {
      let dayInWeekStrOfActiveDate = utils.formatByString(
        dateToTransferFrom,
        section.format,
      );
      if (section.hasLeadingZerosInInput) {
