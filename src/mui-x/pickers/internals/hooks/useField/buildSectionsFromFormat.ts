    ...
    if (hasLeadingZerosInFormat) {
      ...
      if (isValidDate) {
        sectionValue = applyLocalizedDigits(
          cleanLeadingZeros(
            removeLocalizedDigits(sectionValue, localizedDigits),
            maxLength,
          ),
          localizedDigits,
        );
...
const buildSections = (
  ...
  const validTokens = Object.keys(utils.formatTokenMap).sort(
    (a, b) => b.length - a.length,
  ); // Sort to put longest word first

  const regExpFirstWordInFormat = /^([a-zA-Z]+)/;
  const regExpWordOnlyComposedOfTokens = new RegExp(
    `^(${validTokens.join("|")})*$`,
  );
  ...
  while (i < expandedFormat.length) {
    ...
    const firstWordInFormat = regExpFirstWordInFormat.exec(
      expandedFormat.slice(i),
    )?.[1];
