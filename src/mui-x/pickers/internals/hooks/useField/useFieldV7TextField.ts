...
>(
  ...
  const handleRootKeyDown = useEventCallback(
  ...
  const handleRootBlur = useEventCallback(
  ...
  const handleRootFocus = useEventCallback(
    (event: React.FocusEvent<HTMLDivElement>) => {
      onFocus?.(event);
  ...
  const handleRootPaste = useEventCallback(
  ...
  const handleRootInput = useEventCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      onInput?.(event);
  ...
  useEnhancedEffect(() => {
    ...
    if (parsedSelectedSections === "all") {
      ...
      const domElement = sectionListRef.current.getSectionContent(
        parsedSelectedSections,
      );
      if (domElement) {
        domElement.focus();
  ...
  return {
    ...
    error,
    clearable: Boolean(
      clearable && !areAllSectionsEmpty && !readOnly && !disabled,
    ),
...
function getActiveSectionIndex(
  sectionListRef: React.RefObject<PickersSectionListRef | null>,
) {
