...
>(
  ...
  const localizedDigits = React.useMemo(
  ...
  const [state, setState] = React.useState<UseFieldState<TValue>>(() => {
    ...
    const stateWithoutReferenceDate: Omit<
  ...
  const activeSectionIndex =
    parsedSelectedSections === "all" ? 0 : parsedSelectedSections;

  const sectionOrder = React.useMemo(
    () =>
      getSectionOrder(
        state.sections,
        isRtl && !enableAccessibleFieldDOMStructure,
      ),
  ...
  const setSectionUpdateToApplyOnNextInvalidDate = (
  ...
  const clearActiveSection = () => {
    ...
    if (fieldValueManager.getDateFromSection(value, activeSection) === null) {
      ...
      setState((prevState) => ({ ...prevState, characterQuery: null }));
      publishValue(
        fieldValueManager.updateDateInValue(value, activeSection, null),
      );
    }
  };
  ...
  const updateSectionValue = ({
    ...
    const newActiveDateSections = fieldValueManager.getDateSectionsFromValue(
      newSections,
      section,
    );
    ...
    if (utils.isValid(newActiveDate)) {
      ...
      if (activeDate == null) {
        cleanActiveDateSectionsIfValueNullTimeout.start(0, () => {
          if (valueRef.current === value) {
            setState((prevState) => ({
              ...prevState,
              sections: fieldValueManager.clearDateSections(
                state.sections,
                section,
              ),
              tempValueStrAndroid: null,
      ...
      return publishValue(
        fieldValueManager.updateDateInValue(value, section, mergedDate),
      );
    }
    ...
    if (newActiveDateSections.every((sectionBis) => sectionBis.value !== "")) {
      ...
      return publishValue(
        fieldValueManager.updateDateInValue(value, section, newActiveDate),
      );
    }
    ...
    if (activeDate != null) {
      setSectionUpdateToApplyOnNextInvalidDate(newSectionValue);
      return publishValue(
        fieldValueManager.updateDateInValue(value, section, null),
      );
    }
  ...
  const setCharacterQuery = useEventCallback(
    (newCharacterQuery: CharacterEditingQuery | null) => {
      setState((prevState) => ({
        ...prevState,
  ...
  if (
    ...
    state.sections[state.characterQuery.sectionIndex]?.type !==
      state.characterQuery.sectionType
  ) {
    setCharacterQuery(null);
  }
...
export interface UseFieldStateReturnValue<TValue extends PickerValidValue> {
  ...
  updateSectionValue: (
    parameters: UpdateSectionValueParameters<TValue>,
  ) => void;
