...
>): UsePickerReturnValue<TValue> => {
  ...
  const fieldRef = React.useRef<
    FieldRef<PickerValue> | FieldRef<PickerRangeValue> | null
  >(null);
  ...
  const { hasUIView, viewModeLookup, timeViewsCount } = React.useMemo(
    () =>
      views.reduce(
        (acc, viewForReduce) => {
          const viewMode =
            viewRenderers[viewForReduce] == null ? "field" : "UI";
  ...
  const actionsContextValue = React.useMemo<
    PickerActionsContextValue<TValue, TView, TError>
  >(
    () => ({
      setValue,
      setOpen,
  ...
  const fieldPrivateContextValue =
    React.useMemo<PickerFieldPrivateContextValue>(
      () => ({
        formatDensity,
        enableAccessibleFieldDOMStructure,
        selectedSections,
