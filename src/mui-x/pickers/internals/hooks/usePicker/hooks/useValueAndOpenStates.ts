...
>(parameters: UsePickerDateStateParameters<TValue, TView, TExternalProps>) {
  ...
  const setValue = useEventCallback(
    (newValue: TValue, options?: SetValueActionOptions<TError>) => {
      const {
      ...
      if (
        ...
        !isValueControlled &&
        !state.hasBeenModifiedSinceMount
      ) {
      ...
      setState((prevState) => ({
        ...
        clockShallowValue: shouldFireOnChange
          ? undefined
          : prevState.clockShallowValue,
        lastCommittedValue: shouldFireOnAccept
          ? value
          : prevState.lastCommittedValue,
  ...
  const setValueFromView = useEventCallback(
    (newValue: TValue, selectionState: PickerSelectionState = "partial") => {
      ...
      setValue(newValue, {
        changeImportance:
          selectionState === "finish" && closeOnSelect ? "accept" : "set",
      });
    },
  );
