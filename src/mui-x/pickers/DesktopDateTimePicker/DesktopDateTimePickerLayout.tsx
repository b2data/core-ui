 ...
 */
const DesktopDateTimePickerLayout = React.forwardRef(
  function DesktopDateTimePickerLayout<TValue extends PickerValidValue>(
    props: PickersLayoutProps<TValue>,
    ref: React.Ref<HTMLDivElement>,
  ) {
    const { toolbar, tabs, content, actionBar, shortcuts, ownerState } =
      usePickerLayout(props);
    const { orientation } = usePickerContext();
    const { sx, className, classes } = props;
    const isActionBarVisible =
      actionBar && (actionBar.props.actions?.length ?? 0) > 0;
    ...
    return (
      ...
      >
        ...
        >
          ...
          {tabs}
          {isActionBarVisible && (
            <Divider sx={{ gridRow: 3, gridColumn: "1 / 4" }} />
          )}
        </PickersLayoutContentWrapper>
        {actionBar}
      </PickersLayoutRoot>
