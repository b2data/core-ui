...
const huHUPickers: Partial<PickersLocaleText> = {
  ...
  minutesClockNumberText: (minutes) =>
    `${minutes} ${timeViews.minutes.toLowerCase()}`,
  secondsClockNumberText: (seconds) =>
    `${seconds}  ${timeViews.seconds.toLowerCase()}`,
  ...
  openTimePickerDialogue: (formattedTime) =>
    formattedTime
      ? `Válasszon időt, a kiválasztott idő: ${formattedTime}`
      : "Válasszon időt",
  ...
  fieldMonthPlaceholder: (params) =>
    params.contentType === "letter" ? "HHHH" : "HH",
  fieldDayPlaceholder: () => "NN",
  fieldWeekDayPlaceholder: (params) =>
    params.contentType === "letter" ? "NNNN" : "NN",
