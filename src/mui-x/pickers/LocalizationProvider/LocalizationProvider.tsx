...
export type MuiPickersAdapterContextNullableValue = {
  [K in keyof MuiPickersAdapterContextValue]:
    | MuiPickersAdapterContextValue[K]
    | null;
};
...
export const LocalizationProvider = function LocalizationProvider<TLocale>(
  ...
  const defaultDates: MuiPickersAdapterContextNullableValue["defaultDates"] =
    React.useMemo(() => {
      if (!utils) {
        return null;
      }
  ...
  const contextValue: MuiPickersAdapterContextNullableValue =
    React.useMemo(() => {
      return {
        utils,
        defaultDates,
