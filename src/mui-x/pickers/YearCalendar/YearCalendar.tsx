...
const YearCalendarRoot = styled("div", {
  ...
  shouldForwardProp: (prop) =>
    shouldForwardProp(prop) && prop !== "yearsPerRow",
...
export const YearCalendar = React.forwardRef(function YearCalendar(
  ...
  const handleYearSelection = useEventCallback(
    (_event: React.MouseEvent, year: number) => {
      if (readOnly) {
        return;
      }
  ...
  const verticalDirection =
    yearsOrder !== "desc" ? yearsPerRow * 1 : yearsPerRow * -1;
  const horizontalDirection =
    (isRtl && yearsOrder === "asc") || (!isRtl && yearsOrder === "desc")
      ? -1
      : 1;
  ...
  const handleYearFocus = useEventCallback(
    (_event: React.FocusEvent, year: number) => {
      focusYear(year);
    },
  );
