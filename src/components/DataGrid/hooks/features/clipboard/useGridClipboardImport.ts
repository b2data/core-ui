...
import { GRID_REORDER_COL_DEF } from "../rowReorder";
...
class CellValueUpdater {
  ...
  options: {
    ...
    processRowUpdate: DataGridProcessedProps["processRowUpdate"];
    onProcessRowUpdateError: DataGridProcessedProps["onProcessRowUpdateError"];
  ...
  applyUpdates() {
    ...
    if (rowIdsToUpdate.length === 0) {
      apiRef.current.publishEvent("clipboardPasteEnd", { status: "error" });
      return;
    }
    ...
    Promise.all(promises).then(() => {
      this.rowsToUpdate = {};
      apiRef.current.publishEvent("clipboardPasteEnd", { status: "success" });
    });
  }
}
...
function defaultPasteResolver({
  ...
  pagination: DataGridProcessedProps["pagination"];
}) {
  ...
  if (
    cellSelectionModel &&
    apiRef.current.unstable_getSelectedCellsAsArray().length > 1
  ) {
...
export const useGridClipboardImport = (
  ...
  const enableClipboardPaste =
    (!props.disableClipboardPaste && props.clipboardPaste) ?? false;
