...
import {
  DataSourceCache,
  DataSourceCacheDefault,
} from "@mui/x-tree-view/utils";
import { NestedDataManager } from "../../../mui-x/tree-view-pro/internals/plugins/useTreeViewLazyLoading/utils";
...
/**
 * Custom lazy-loading plugin that allows the initial tree to be hydrated from `items`
 * (for restored state) and only enables lazy loading for subsequent expansions.
 */
export const useInitialTreeViewLazyLoading: TreeViewPlugin<
  UseTreeViewLazyLoadingSignature
> = ({ instance, params, store }) => {
  const isLazyLoadingEnabled = !!params.dataSource;
  const firstRenderRef = React.useRef(true);
  const updatesPreventedRef = React.useRef(false);

  const nestedDataManager = useLazyRef<NestedDataManager, void>(
    () => new NestedDataManager(instance),
  ).current;

  const cacheRef = useLazyRef<DataSourceCache, void>(() =>
    getCache(params.dataSourceCache),
  );

  const preventItemUpdatesOnce = React.useCallback(() => {
    if (!updatesPreventedRef.current) {
      instance.preventItemUpdates();
      updatesPreventedRef.current = true;
    }
  }, [instance]);
  ...
  const setDataSourceLoading = useEventCallback(
    (itemId: TreeViewItemId, isLoading: boolean) => {
      ...
      if (
        lazyLoadingSelectors.isItemLoading(store.state, itemId) === isLoading
      ) {
        return;
      }
      ...
      store.set("lazyLoading", {
        ...
        dataSource: {
          ...store.state.lazyLoading.dataSource,
  ...
  const fetchItemChildren: UseTreeViewLazyLoadingInstance["fetchItemChildren"] =
    useEventCallback(async ({ itemId, forceRefresh }) => {
      if (!isLazyLoadingEnabled) {
        return;
      }
      const getChildrenCount = params.dataSource?.getChildrenCount || (() => 0);
  ...
  useInstanceEventHandler(
    ...
    async (eventParameters) => {
      ...
      const itemHasError = lazyLoadingSelectors.itemHasError(
        store.state,
        eventParameters.itemId,
      );
      if (!itemHasError) {
        ...
        if (
          selectionSelectors.isItemSelected(store.state, eventParameters.itemId)
        ) {
  ...
  React.useEffect(() => {
    if (isLazyLoadingEnabled && firstRenderRef.current) {
      ...
      if (params.items.length) {
        const getChildrenCount =
          params.dataSource?.getChildrenCount || (() => 0);
        const getItemChildren =
          params.getItemChildren || ((item: any) => item.children);
        const hasChildren = params.items.some((item) => {
          const children = getItemChildren(item);
          return children && children.length > 0;
        });

        if (!hasChildren) {
          // no children provided -> behave like default plugin
          instance.addItems({
            items: params.items,
            depth: 0,
            getChildrenCount,
          });
          preventItemUpdatesOnce();
        } else {
          // allow useTreeViewItems to process provided children first
          setTimeout(() => {
            preventItemUpdatesOnce();
          }, 0);
        }
    ...
    } else if (isLazyLoadingEnabled && !firstRenderRef.current) {
      preventItemUpdatesOnce();
    }
  }, [
    instance,
...
useInitialTreeViewLazyLoading.getInitialState = () => ({
  lazyLoading: {
    enabled: false,
    dataSource: INITIAL_STATE,
  },
...
useInitialTreeViewLazyLoading.params = {
  dataSource: true,
